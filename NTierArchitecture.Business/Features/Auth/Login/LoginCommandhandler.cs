...
namespace NTierArchitecture.Business.Features.Auth.Login
{
    internal sealed class LoginCommandhandler : IRequestHandler<LoginCommand, LoginCommandResponse>
    {
        ...
        {
            AppUser appUser = await _userManager.Users.Where(p=>p.UserName == request.UserNameOrEmail || 
            p.Email == request.UserNameOrEmail).FirstOrDefaultAsync(cancellationToken);
